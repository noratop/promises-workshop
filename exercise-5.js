    ...
    response.forEach(function(elt){
        ...
        if (elt.response){
            ...
            if (elt.response.adverb){
                ...
                if (elt.response.adverb.syn) elt.response.adverb.syn.forEach(function(elt){
                    console.log('- '.bold.magenta+elt.magenta);
                });
            }
            if (elt.response.adjective){
                console.log('As an adjective:'.bold.gray);
                if (elt.response.adjective.syn) elt.response.adjective.syn.forEach(function(elt){
                    console.log('- '.bold.magenta+elt.magenta);
                });
            }
            if (elt.response.noun){
                console.log('As a noun:'.bold.gray);
                if (elt.response.noun.syn) elt.response.noun.syn.forEach(function(elt){
                    console.log('- '.bold.magenta+elt.magenta);
                });
            }
            if (elt.response.verb){
                console.log('As a verb:'.bold.gray);
                if (elt.response.verb.syn) elt.response.verb.syn.forEach(function(elt){
                    console.log('- '.bold.magenta+elt.magenta);
                });   
            }
        }
